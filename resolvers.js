...
const resolvers = {
    Query: {
        ...
        getClientByID: async(_,args) => {
            try {
                const id = args.id;
                const client = await Client.findOne({ id: id }).populate("reservations");
                return client;
            } catch (error) {
    ...
    Mutation: {
        ...
        createReservation: async (_, args) => {
            try {
                const { id,bookingStartDate, bookingEndDate, service, comments,idClient} = args;
                const client = await Client.findOne({ id: idClient });
                if (!client) {
                    throw new Error("Cliente no encontrado");
                }
                const reservation = new Reservation({ id,bookingStartDate, bookingEndDate, service, comments, client: client._id });
